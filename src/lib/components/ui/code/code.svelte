<script lang="ts">
	...
	const codeState = useCode({
		...
		diffAdd: box.with(() => diffAdd),
		diffRemove: box.with(() => diffRemove),
		lang: box.with(() => lang)
	});
</script>

<div {...rest} bind:this={ref} class={cn(codeVariants({ variant }), className)}>
	{#if codeState.highlighted}
		<!-- eslint-disable-next-line svelte/no-at-html-tags -->
		{@html codeState.highlighted}
	{:else}
		<pre class="shiki overflow-x-auto rounded-lg py-4 text-sm"><code class="grid min-w-full break-words bg-transparent p-0">{codeState.code}</code></pre>
	{/if}
	{@render children?.()}
</div>
