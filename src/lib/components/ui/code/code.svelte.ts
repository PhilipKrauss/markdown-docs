			...
			transformers: [
				{
					...
					line: (node, line) => {
						...
						if (this.opts.diffAdd.current.includes(line)) {
							node.properties.class = cls + ' diff-add';
							node.properties.style = 'background-color:color-mix(in oklab,var(--success) 22%,transparent)';
						} else if (this.opts.diffRemove.current.includes(line)) {
							node.properties.class = cls + ' diff-remove';
							node.properties.style = 'background-color:color-mix(in oklab,var(--error) 22%,transparent)';
						} else if (within(line, this.opts.highlight.current)) {
							node.properties.class = cls + ' line--highlighted';
						}
						return node;
					}
				}
