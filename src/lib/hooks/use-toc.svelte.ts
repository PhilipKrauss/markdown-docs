...
/** Returns a copy of the TOC tree keeping only headings up to `maxLevel`. */
export function filterTocByLevel(toc: Heading[], maxLevel: number): Heading[] {
	return toc.map((h) => ({
		...h,
		children: h.level < maxLevel ? filterTocByLevel(h.children, maxLevel) : [],
	}));
}
